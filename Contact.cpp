...
void Contact::SEARCH(Contact contact[9])
{
	...
	{
		std::cout << fix_string(contact[i].index);
		std::cout << fix_string(contact[i].first_name);
		std::cout << fix_string(contact[i].last_name);
		std::cout << fix_string(contact[i].nickname) << std::endl;
		i++;
	}
	std::cout << "Enter the index of the record to be retrieved: ";
	std::cin >> record_number;
	while (record_number >= 9 || (contact[record_number - 1].index).length() == 0)
	{
